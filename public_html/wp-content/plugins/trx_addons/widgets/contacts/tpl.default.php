...
if ($show_info || !empty($googlemap)) {
	...
	if (!empty($address) || !empty($phone) || !empty($email)) {
        ...
        if (!empty($phone) || !empty($email)) {
            ...
            if (!empty($phone)) {
                ?><span class="contacts_phone"><a href="tel:<?php echo wp_kses_data($phone); ?>"><?php echo wp_kses_data($phone); ?></a></span></span><?php
            }
            if ($columns) {
                ?></div><?php
            }
			...
			if (!empty($address)) {
				...
				?><span class="contacts_address"><?php echo str_replace('|', "<br><span>", $address); ?></span></span><?php
				if ($columns) {
					?></div><?php
				}
			}
