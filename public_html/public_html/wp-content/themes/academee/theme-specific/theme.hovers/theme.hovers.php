...
if (!function_exists('academee_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'academee_hovers_theme_setup3', 3 );
	function academee_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		academee_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'academee'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'academee') ),
				"std" => 'fade',
				"options" => array(
					'default'		=> esc_html__('Fade',				'academee'),
			...
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'academee'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'academee') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'academee'),
					'icon'	=> esc_html__('Icon',	'academee'),
					'icons'	=> esc_html__('Icons',	'academee'),
					'zoom'	=> esc_html__('Zoom',	'academee'),
					'fade'	=> esc_html__('Fade',	'academee'),
					'slide'	=> esc_html__('Slide',	'academee'),
					'pull'	=> esc_html__('Pull',	'academee'),
					'border'=> esc_html__('Border',	'academee')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('academee_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'academee_hovers_theme_setup9', 9 );
	function academee_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'academee_hovers_frontend_scripts', 1010 );
		add_filter( 'academee_filter_localize_script','academee_hovers_localize_script' );
		add_filter( 'academee_filter_merge_scripts',	'academee_hovers_merge_scripts' );
		add_filter( 'academee_filter_merge_styles',	'academee_hovers_merge_styles' );
		add_filter( 'academee_filter_get_css', 		'academee_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'academee_hovers_frontend_scripts' ) ) {
	
	function academee_hovers_frontend_scripts() {
		if ( academee_is_on(academee_get_theme_option('debug_mode')) && academee_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'academee-hovers', academee_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( academee_is_on(academee_get_theme_option('debug_mode')) && academee_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'academee-hovers',  academee_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('academee_hovers_merge_scripts')) {
	
	function academee_hovers_merge_scripts($list) {
...
if (!function_exists('academee_hovers_merge_styles')) {
	
	function academee_hovers_merge_styles($list) {
...
if (!function_exists('academee_hovers_localize_script')) {
	
	function academee_hovers_localize_script($arr) {
		$arr['button_hover'] = academee_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('academee_hovers_add_icons') ) {
	function academee_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'academee') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'academee');
						else
							esc_html_e('Details', 'academee');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-plus"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'academee_hovers_get_css' ) ) {
	
	function academee_hovers_get_css($css, $colors, $fonts, $scheme='') {
